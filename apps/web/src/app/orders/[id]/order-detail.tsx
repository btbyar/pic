...
export function OrderDetail({ id }: { id: string }) {
  ...
  return (
    <main className="mx-auto flex max-w-3xl flex-col gap-5 px-4 py-8">
      <Link href="/my/orders" className="text-sm text-slate-600 underline-offset-4 hover:underline">
        ← {t('myOrders')}
      </Link>
...
function PaymentCard({ order, token, onChange }: { order: OrderView; token: string; onChange: () => Promise<void> }) {
  ...
  return (
    <Card className="flex flex-col items-center gap-4 text-center">
      ...
      <p className="text-sm text-slate-600">{t('payHint')}</p>
      {qr ? (
        <img src={qr} alt={t('qrAlt')} width={280} height={280} className="rounded-xl border border-slate-200" />
      ) : (
          ...
          <ul className="grid grid-cols-3 gap-2 sm:grid-cols-4">
            {order.payment.deeplinks.map((bank) => (
              <li key={bank.link}>
                <a
                  href={bank.link}
                  className="flex min-h-20 flex-col items-center justify-center gap-1 rounded-xl border border-slate-200 p-2 text-xs"
                >
      ...
      <div className="flex items-center gap-2 text-sm text-slate-600" role="status">
        <span className="h-4 w-4 animate-spin rounded-full border-2 border-slate-300 border-t-slate-900" aria-hidden />
        {t('waiting')}
  ...
  return (
    <>
      ...
      <Card className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-sm text-slate-700">{order.emailOnFile ? t('keepLinkEmailed') : t('keepLink')}</p>
      ...
      <ul className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        {order.items.map((item, i) => (
          <li key={item.id} className="flex flex-col gap-2">
            {item.thumbUrl ? (
              <img src={item.thumbUrl} alt="" loading="lazy" className="aspect-[4/3] w-full rounded-lg bg-slate-200 object-cover" />
            ) : (
              <div className="flex aspect-[4/3] w-full items-center justify-center rounded-lg bg-slate-100 p-2 text-center text-xs text-slate-500">
