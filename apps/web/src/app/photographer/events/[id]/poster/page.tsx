...
export default async function EventPosterPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      ...
      <p className="max-w-2xl text-sm text-stone-600 print:hidden">
        {event.visibility === 'PUBLIC' ? t('intro') : t('introHidden')}
      </p>

      {/* A4 хуудас: дэлгэц дээр ч, хэвлэхэд ч ижил харьцаа */}
      <div className="mx-auto w-full max-w-[210mm] rounded-2xl border border-stone-200 bg-white p-10 text-center print:max-w-none print:rounded-none print:border-0 print:p-0">
        <div className="flex flex-col items-center gap-6">
          ...
          <div className="flex flex-col gap-1">
            ...
            <p className="text-lg text-stone-600">{formatEventRange(event.startsAt, event.endsAt, event.timezone)}</p>
            {event.location ? <p className="text-lg text-stone-600">{event.location}</p> : null}
          </div>

          <p className="max-w-md text-balance text-2xl font-semibold text-brand-700">{t('headline')}</p>
          ...
          <p className="font-mono text-sm text-stone-500">{url.replace(/^https?:\/\//, '')}</p>

          <ol className="flex max-w-lg flex-col gap-2 text-left text-lg">
            {(['scan', 'selfie', 'buy'] as const).map((k, i) => (
              <li key={k} className="flex gap-3">
                <span className="font-display font-bold tabular-nums text-brand-700">{i + 1}.</span>
                {t(`steps.${k}`)}
              </li>
            ))}
          </ol>
