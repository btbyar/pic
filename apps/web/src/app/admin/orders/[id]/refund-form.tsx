  ...
  return (
    <form onSubmit={(e) => void submit(e)} className="flex flex-col gap-3">
      <ul className="flex flex-col gap-1">
        {items.map((i) => (
          <li key={i.id}>
            <label className={`flex items-center justify-between gap-3 rounded-lg bg-surface-2 px-3 py-2 text-sm ${i.refunded ? 'text-ink-faint line-through' : ''}`}>
      ...
      {selected.size ? (
        <div className="flex flex-col gap-2 rounded-xl border border-red-200 bg-red-50 p-3">
          ...
          <p className="text-xs text-ink-soft">{t('refundManual')}</p>
