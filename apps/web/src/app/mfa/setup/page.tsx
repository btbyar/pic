  ...
  if (recoveryCodes) {
    return (
      <AuthShell title={t('recoveryTitle')} intro={t('recoveryIntro')}>
        <ol className="grid grid-cols-2 gap-2 rounded-2xl border border-stone-200 bg-stone-50 p-4 font-mono text-sm">
      ...
      {setup ? (
        <>
          <div className="flex flex-col items-center gap-3">
            {qr ? <img src={qr} alt="QR" width={220} height={220} className="rounded-xl border border-stone-200" /> : null}
            <p className="text-center text-sm text-stone-600">{t('manualEntry')}</p>
            <code className="break-all rounded-lg bg-stone-100 px-3 py-2 text-sm">{setup.secret}</code>
          </div>
